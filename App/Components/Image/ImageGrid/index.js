...
class ImageGrid extends PureComponent {
  constructor(props) {
    ...
    this.state = {
      ...
      width: METRICS.DEVICE_WIDTH / 2 - 30
    };
  }
  ...
  generateGridData = data => {
    ...
    while (output.length > 0) {
      let gridTemp = output.splice(
        0,
        output.length > columnTotal ? columnTotal : output.length
      );
  ...
  render() {
    ...
    return (
      <View style={[styles.mainContainer, this.props.styleContainer]}>
        ...
        {selectMode && (
          <TouchableOpacity onPress={() => this._onDeleteImage()}>
            <Text>Hapus</Text>
          </TouchableOpacity>
        )}
        <ImageFullScreen
