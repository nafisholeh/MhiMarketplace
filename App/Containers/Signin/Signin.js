...
class Signin extends Component {
  ...
  onSigninFarmer = async () => {
    ...
    try {
      ...
      if (signinFarmer) {
        await storeSession(signinFarmer, Config.userType.FARMER);
        const screenName = "CommoditySocmed";
        const resetAccountStack = StackActions.reset({
          index: 0,
          actions: [NavigationActions.navigate({ routeName: "FarmerProfile" })]
        });
        const goToHome = NavigationActions.navigate({
          routeName: screenName
        });
        navigation.dispatch(resetAccountStack);
...
const mapDispatchToProps = dispatch => ({
  storeSession: (user, user_type) =>
    dispatch(SessionActions.storeSession(user, user_type)),
