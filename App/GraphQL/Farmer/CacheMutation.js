...
export const cacheNewComment = async data => {
  const { postId, commentId, content, author, dateCommented } = data || {};
  const { _id: authorId, ktp_photo_face_thumbnail: ktp_photo_face, ktp_name } =
    author || {};
  const result = await ApolloClientProvider.client.cache.readQuery({
  ...
  ApolloClientProvider.client.writeQuery({
    ...
    data: {
      farmerPost: {
        ...farmerPost,
        ...{
          comments: [].concat(comments, newComment)
        }
      }
    }
  });
...
export const cacheNewReplyComment = async data => {
  const { postId, commentId, subCommentId, content, author, dateCommented } =
    data || {};
  const { _id: authorId, ktp_photo_face_thumbnail: ktp_photo_face, ktp_name } =
    author || {};
  const result = await ApolloClientProvider.client.cache.readQuery({
