...
import { NewsFeedContent, CommentItem, CommentInput } from "./Components";
...
class NewsFeedDetail extends Component {
  ...
  submitCommentToPost = (feedId, feedAuthorId, comment) => {
    ...
    ApolloClientProvider.client
      .mutate({
        ...
        optimisticResponse: isReplyComment
          ? {
  ...
  render() {
    ...
    return (
      <View style={{ flex: 1 }}>
        <Query query={FETCH_FARMER_POST} variables={{ _id: feedId }}>
          {({ loading, error, data, refetch }) => {
            ...
            if (dataList) {
              ...
              const { _id: feedAuthorId, ktp_name, ktp_photo_face } =
                author || {};
              return (
                <Fragment>
                  <CustomScrollView
